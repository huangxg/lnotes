...
行间公式和无编号独立公式都有多种输入方法，新手也许会看花了眼。懒人包老师的秘诀是用最短的：行间公式用 \verb|$...$|，无编号独立公式用 \verb|\[...\]|。建议不要用 \verb|$$...$$|，因为它和 \AmSLaTeX 有冲突。\texttt{ams\-math} 版本的 \texttt{equation} 环境可以嵌入次环境 (见 \ref{sec:longeq} 小节) 。
